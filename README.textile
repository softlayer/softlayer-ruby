...
<div id="note" style="margin: 1em; padding: 1em; background-color: #fcfcfc; border: 1px solid black">
	*Important Note*
...
The Ruby client has been tested using a wide variety of Ruby implementations including Ruby version 1.8.7. 1.9.2, 2.0, and 2.1. It has also been tested on JRuby running in 1.8 and 1.9 mode.
...
Version 2.0 of the softlayer_api gem changes the protocol used to communicate with the SoftLayer API from REST to XML-RPC. This change should not require any changes to existing scripts. However there are two more changes which may require changes to scripts.  First, Object Masks are now sent to the server using the "Extended Object Mask Format":http://sldn.softlayer.com/article/Object-Masks. Secondly, the software also changes the syntax for specifying result limits.

h3. XML-RPC

When sending requests to the server, an API client may use SOAP:http://sldn.softlayer.com/article/SOAP, XML-RPC:http://sldn.softlayer.com/article/XML-RPC, or REST:http://sldn.softlayer.com/article/REST. Previous versions of the SoftLayer API Gem used the REST to communicate with the server. This version uses XML-RPC. Adopting the XML-RPC transport mechanism allows the Gem to take adavantage of features of the API, most notalby Object Filters, that are not available through the REST interface.

h3. Extended Object Masks

The SoftLayer API Gem now sends object masks to the API server using the Extended Object Mask format. This may require you to change existing scripts to retrieve the same information. For example, in prior versions of the Gem, one could retrieve the @metricTrackingObjectId@ for a hardware server by using an object mask with that property to an object managed by the @SoftLayer_Hardware@ service:

<pre style="border:1pt solid black;background:#eee;padding:0.5em;margin:0.5em"><code style="font-family:Menlo,Monaco,monospace;font-size:9pt">hardware_service = SoftLayer::Service.new('SoftLayer_Hardware')
hardware_record = $softlayer_hardware.object_with_id(123456).object_mask("metricTrackingObjectId").getObject()
</code></pre>

In this version of the Gem, however, this will return an error:

<pre style="border:1pt solid black;background:#eee;padding:0.5em;margin:0.5em"><code style="font-family:Menlo,Monaco,monospace;font-size:9pt">Property 'metricTrackingObjectId' not valid for 'SoftLayer_Hardware'.
</code></pre>

This error comes up because the @metricTrackingObjectId@ property belongs to the @SoftLayer_Hardware_Server@ service, not @SoftLayer_Hardware@.

To make this request work in the 2.0 Gem, you must create a object mask property that instructs the gem which service to look in for the property. This code creates a Extended Object Mask with the type included and avoids the error:

<pre style="border:1pt solid black;background:#eee;padding:0.5em;margin:0.5em"><code style="font-family:Menlo,Monaco,monospace;font-size:9pt">metric_tracking_property = SoftLayer::ObjectMask.new()
metric_tracking_property.type = "SoftLayer_Hardware_Server"
metric_tracking_property.subproperties = "metricTrackingObjectId"

starting_hardware = hardware_service.object_with_id(186908).object_mask(metric_tracking_property).getObject()
</code></pre>

If you find that some of your object masks are no longer working, check the SLDN web site and carefully examine which service provides the properties you are requesting.  Use typed object masks to explicitly retrieve the properties from the correct service.
...
table{position:relative;left:2em;width:80%}.
|{vertical-align:baseline;width:35%}.@API_PUBLIC_ENDPOINT@|A constant containing the base address for the public network REST inspired endpoint of the SoftLayer API - @https://api.softlayer.com/soao/v3/@|
|{vertical-align:baseline}.@API_PRIVATE_ENDPOINT@|A constant containing the base address for the private network REST inspired endpoint of the SoftLayer API - @https://api.service.softlayer.com/soap/v3/@|
